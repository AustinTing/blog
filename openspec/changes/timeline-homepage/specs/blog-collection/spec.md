...
- `category`：enum（`work` | `life`），必填
- `cover`：string，選填，無預設值（圖片路徑，相對於 `public/`）
- `tags`：string array，選填，無預設值

collection 設定 SHALL 接受 `.mdx` 檔案作為內容來源。

#### Scenario: 完整 frontmatter 的文章通過驗證
- **WHEN** 一篇 MDX 文章包含有效的 `title`、`description`、`date`、`category` frontmatter
- **THEN** 系統將其納入 `blog` collection，`entry.data` 包含所有已驗證的欄位

#### Scenario: 缺少必填欄位時擲出驗證錯誤
- **WHEN** 一篇 MDX 文章缺少 `title` 或 `description` 或 `date` 或 `category`
...
- **WHEN** 一篇 MDX 文章的 frontmatter 包含 `category: work` 或 `category: life`
...
- **WHEN** 一篇 MDX 文章的 frontmatter 包含不在 enum 清單內的 `category` 值（例如 `learning`）
- **THEN** Astro 在建置或開發模式下擲出 content collection 驗證錯誤

#### Scenario: 設定 cover 圖片路徑
- **WHEN** 一篇 MDX 文章的 frontmatter 包含 `cover: /images/covers/example.jpg`
- **THEN** `entry.data.cover` 的值為該字串

#### Scenario: 未設定 cover 欄位
- **WHEN** 一篇 MDX 文章的 frontmatter 未包含 `cover` 欄位
- **THEN** `entry.data.cover` 的值為 `undefined`，不擲出驗證錯誤

#### Scenario: 設定 tags 欄位
- **WHEN** 一篇 MDX 文章的 frontmatter 包含 `tags: [astro, htmx]`
- **THEN** `entry.data.tags` 的值為 `["astro", "htmx"]`

#### Scenario: 未設定 tags 欄位
- **WHEN** 一篇 MDX 文章的 frontmatter 未包含 `tags` 欄位
- **THEN** `entry.data.tags` 的值為 `undefined`，不擲出驗證錯誤
